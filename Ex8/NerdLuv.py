#
#    Filename: NerdLuv.py
#    Description: Finished additional features are as follows:
#                     1. server-side form validation,
#                     2. ability to upload photos,
#                     3. ability for returning users to view their matches
#    Last modified: 2014-12-02 10:26
#
...
def storeImage(pic_dict_list):
    for pic_dict in pic_dict_list:
        filename = pic_dict["filename"].replace(' ', '_').lower()
        fs = open("static/images/%s" % filename, "wb")
        fs.write(pic_dict["body"])
        fs.close()
...
class ResultHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        if "Pic" in self.request.files:
            pic_dict_list = self.request.files["Pic"]
        else:
            pic_dict_list = None;
        ...
        if isError:
            ...
            if pic_dict_list: # if user uploads images, put the picture into "static/images" path.
                storeImage(pic_dict_list)
